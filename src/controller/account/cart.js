...
export default class Cart extends Base {
  ...
  async delcartAction() {
    ...
    if (this.isAjax("post")) {
      ...
      for (let val of ids.split("<>")) {
        ...
        let id = await this.model("cart").where({
          product_id: val.split("||")[0],
          type: val.split("||")[1] || "",
          uid: this.user.uid
        }).delete();
      }
      if (this.isMobile) {
        return this.success({name: "删除成功！", url: "/account/cart/index"})
      } else {
      ...
      this.meta_title = "删除";
      this.active = "/account/cart/index";
      //判断浏览客户端
      if (this.isMobile) {
        return this.display(this.mtpl())
      } else {
  ...
  async getPayOverview (goods) {
    ...
    for (let fa of farr) {
      let fobj = {};
      fobj.id = fa;
      fobj.cg = think._.filter(goods, ['fare', fa]);
      cgarr.push(fobj);
    }
  ...
  async createorderAction() {
    ...
    //生成订单编号
    data.order_no = await this.model("order").getOrderid(this.user.uid);
    //添加送货地址

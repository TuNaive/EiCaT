...
function _address() {
  ...
  /** ADD TO COMPARE
   ** *********************** **/
  $("a.add-compare").bind("click", function(e) {
    e.preventDefault();
  ...
  /**
   @COLOR SELECTOR
   **/
  $("#product-color-dd li a").bind("click", function(e) {
    e.preventDefault();

    var data_val 	= $(this).attr('data-val').trim();
    _color 		= $(this).attr('href').trim();

    /* change visual value and hidden input */
    $("#product-selected-color").css({"background-color":_color});
    $("#color").val(data_val); // UPDATE HIDDEN FIELD
  });
  ...
  /**
   @SIZE SELECTOR
   **/
  $("#product-size-dd li a").bind("click", function(e) {
    e.preventDefault();

    var data_val = $(this).attr('data-val').trim();

    /* change visual value and hidden input */
    $("#product-selected-size>span").empty().append(data_val);
    $("#size").val(data_val); // UPDATE HIDDEN FIELD

    /* change visual selected */
    $("#product-size-dd li").removeClass('active');
  ...
  /**
   @QTY SELECTOR
   **/
  $("#product-qty-dd li a").bind("click", function(e) {
    e.preventDefault();
    ...
    $("#qty").val(data_val); // UPDATE HIDDEN FIELD
  ...
  $(document).on("click",'a.is_d',function (e) {
    ...
    $.ajax({
      url:"/account/address/addrisdefault",
      data:{id:id},
      success: function (res) {
        if(res.errno == 0){
  ...
  function _addrs(boll) {
    ...
    function ajaxaddr(addr_id) {
      $.ajax({
        url:"/account/address/getaddr/?id="+addr_id,
        success:function (res) {
          //console.log(res);
          if(res.errno==0){
  ...
  function addr_add_html(data){
    ...
    $.each(data,function (k,val) {
      ...
      var html = '<div class="icheck addr-list m-b">'+
        ...
        ' <a  href="/account/address/editaddrmodal/?id='+val.id+'" class="btn btn-default btn-xs edit"  data-toggle="ajaxModal" ><i class="fa fa-edit white" ></i> 编辑 </a>'+
  ...
  $("#accountswitch").bind("click", function() {
    $('#newaccount').slideToggle(200);
  });

  // Shipping Address show|hide
  $("#shipswitch").bind("click", function() {
    $('#shipping').slideToggle(200, function() {
  ...
  $("#payment_check").bind("click", function() {
    $("#ccPayment").slideUp(200);
  });

  // Credit Card
  $("#payment_card").bind("click", function() {
    $("#ccPayment").slideDown(200);
  });
}